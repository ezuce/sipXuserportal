...
uw.service('restService', [
  ...
  function ($rootScope, $http, $q, $sce, request, xmlParser, sharedFactory, CONFIG, _) {
    ...
    function initSipRest() {
      return {
        ...
        postBasicCall: function (to) {
          return request(authHeaders({
            method:  'POST',
            url:     baseRest + '/my/redirect/callcontroller/' + self.cred.user + '/' + to
          }))
        },
        ...
        putPassword: function (pass) {
          return request(authHeaders({
            method:  'PUT',
            url:     baseRest + '/my/portal/password/' + pass,
            data:    {}
          }))
        },
        getImBot: function () {
    ...
    function parseXml (xml, cb) {
      var $             = angular.element;
      var children      = xmlParser.parse(xml).children ? $(xmlParser.parse(xml).children)[0].children : null;
      var arr           = [];
      var attrs         = [];
      var obj           = {};
      var subChildren   = [];
    ...
    function authHeaders(conf) {
      var b64 = Base64.encode(self.cred.user + ':' + self.cred.pass);
