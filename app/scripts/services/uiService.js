(function(){
  ...
  uw.service('uiService', [
    ...
    'restService',
    '_',
    function ($rootScope, $interval, sharedFactory, restService, _) {
      ...
      var secondary = {
        ...
        logout : {
          init : function () {
            window.location.assign('/sipxconfig/user_portal/EditMyInformation,$common$Border.logoutLink.sdirect');
          }
        },
        ...
        voicemail: {
          ...
          init: function () {
            ...
            if (!_.isEmpty(secondary.voicemail.folder)) {
              ...
              restService.getMessages(temp).then(function (messages) {
                secondary.voicemail.messages = messages;
              })
            } else {
          ...
          getMessage: function (id, index) {
            ...
            restService.getMessage(id)
              .then(function (url) {
                secondary.voicemail.messages[index].href   = url;
                delete secondary.voicemail.messages[index].loading;
                if (!secondary.voicemail.messages[index].heard) {
                  return restService.heardMessage(id);
                }
              })
              .then(function () {
                if (!secondary.voicemail.messages[index].heard) {
                  return restService.getHeardMessage(id);
                }
              })
              .then(function () {
                secondary.voicemail.messages[index].heard  = true;
                delete secondary.voicemail.messages[index].loading;
              });
          },
          ...
          delMessage: function (id, index) {
            ...
            if (secondary.voicemail.folder.name === 'deleted') {
              restService.delMessage(id).then(function () {
                secondary.voicemail.messages.splice(index, 1);
              }, function (er) {
                console.log(er);
              });
            } else {
          ...
          moveMessage: function (id, index) {
            var folder = angular.copy(secondary.voicemail.moveVoicemail[index]);
            secondary.voicemail.messages[index].loading = true;
        ...
        settings: {
          ...
          personalAttendant: {
            ...
            save: function () {
              var main = secondary.settings.personalAttendant.main;
          ...
          fwd: {
            ...
            setup: {
              ...
              add: function () {
                ...
                secondary.settings.fwd.setup.custom.push(angular.copy(secondary.settings.fwd.setup.default));
              },
              ...
              save: function () {
                var custom = angular.copy(secondary.settings.fwd.setup.custom);
                if (custom.length > 0) {
                  _.each(custom, function (obj) {
                    secondary.settings.fwd.setup.resp.rings.push(obj);
                  })
                  secondary.settings.fwd.setup.custom = [];
                }
                secondary.settings.success = false;
